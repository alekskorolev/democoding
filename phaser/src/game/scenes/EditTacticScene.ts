...
const CTX_MENU: Array<MenuItem> = [
  ['save', 'Сохранить'],
];
export default class EditTacticScene extends Scene {
  back?: GameObjects.Image;

  constructor(config: Types.Scenes.SettingsConfig) {
    super({ key: 'edit-tactic', ...config });
  }

  preload() {
    this.load.setBaseURL(`${GAME_ASSETS_URL}/game/`);
    this.load.image('btn', 'ui/btn.png');
  ...
  create() {
    ...
    this.input.on('pointerup', (event: Input.Pointer) => {
      if (event.rightButtonReleased()) {
        const { worldX, worldY } = event;
        const menu = new ContextMenu(this, CTX_MENU, worldX, worldY);
        console.log(event);
        return menu;
      }
      return null;
